...
const emptyFilters = { branch: "All", group: "All", category: "All" };
...
export default function Reports() {
  ...
  const { inwardEntries, outwardEntries, inGold, inSilver, outGold, outSilver } = useMemo(() => {
    const matches = (e) =>
      (applied.branch === "All" || e.branch === applied.branch) &&
  ...
  return (
    <div className="lux-container">
      ...
      <PageHero
        eyebrow="Insights"
        title="Reports"
        subtitle="Today's Gold & Silver Inward and Outward across the network"
      />
      ...
      <GlassCard className="mb-3">
        ...
        <div className="filter-panel mb-3">
          <div>
            ...
            <select className="glass-input glass-select" value={draft.branch} onChange={(e) => setDraft({ ...draft, branch: e.target.value })}>
              <option>All</option>
              {branches.map((b) => (
      ...
      <div className="row g-3 mb-3">
        <div className="col-lg-7">
          <GlassCard>
            ...
            <div className="data-table-head" style={{ gridTemplateColumns: "1.6fr 1fr 1fr 1fr 1fr 0.8fr" }}>
              <span>Branch</span>
              <span>Group</span>
              <span>Category</span>
              <span>Qty</span>
            ...
            {inwardEntries.map((e) => (
              <div key={e.id} className="inventory-row" style={{ gridTemplateColumns: "1.6fr 1fr 1fr 1fr 1fr 0.8fr" }}>
                <div className="inventory-row__value" data-label="Branch" style={{ fontSize: "0.8rem" }}>{e.branch}</div>
            ...
            {inwardEntries.length === 0 && <div className="empty-state">No inward movements match these filters.</div>}
          </GlassCard>
        </div>
      ...
      <GlassCard>
        ...
        {outwardEntries.length === 0 && <div className="empty-state">No outward movements match these filters.</div>}
      </GlassCard>
    </div>
  );
}
